...
import { Calendar, Plus, Trash2, Users, Clock, Building2, Vote, Eye, ArrowRight } from 'lucide-react';
...
const PollDisplay = ({ poll, onBack}) => {
  ...
  const onVote=async(pollId,index)=>{
    try {
      ...
      const res=await axios.post(`${Baseurl}/voting/vote`,{pollId,index,userId})
      // console.log(res);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-800 mb-4">Poll Options</h2>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-semibold text-gray-700">{voteCount} votes</div>
          ...
          {status === 'active' && (
            <div className="mt-8 p-4 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-800 font-medium">This poll is currently active. You can vote now!</p>
            </div>
          )}
          {status === 'upcoming' && (
            <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-yellow-800 font-medium">This poll hasn't started yet. Check back later!</p>
            </div>
          )}
          {status === 'ended' && (
            <div className="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-lg">
              <p className="text-gray-800 font-medium">This poll has ended. Final results are displayed above.</p>
            </div>
          )}
...
const PollCard = ({ poll, onView }) => {
  ...
  const totalVotes = poll.Participate.reduce((sum, option) => sum + (option.count?.length || 0), 0);
    ...
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 p-6 border border-gray-100">
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <h3 className="text-xl font-bold text-gray-800 mb-2 line-clamp-2">{poll.Title}</h3>
        </div>
      </div>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center text-gray-600">
          <Clock className="w-4 h-4 mr-2" />
          <span className="text-sm">
            Starts: {new Date(poll.Starttime).toLocaleDateString()} {new Date(poll.Starttime).toLocaleTimeString()}
          </span>
        </div>
        <div className="flex items-center text-gray-600">
          <Calendar className="w-4 h-4 mr-2" />
          <span className="text-sm">
            Ends: {new Date(poll.Endtime).toLocaleDateString()} {new Date(poll.Endtime).toLocaleTimeString()}
          </span>
        </div>
        <div className="flex items-center text-gray-600">
          <Users className="w-4 h-4 mr-2" />
          <span className="text-sm">{totalVotes} votes • {poll.Optionsize} options</span>
        </div>
      </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 py-8 px-4">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl font-bold text-gray-800 mb-4">Poll Creator</h1>
              <p className="text-gray-600 mb-8 text-lg max-w-2xl mx-auto">
                Create engaging polls for your organization and gather valuable insights from your audience.
              </p>
              <button
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {polls.map((poll, index) => (
                    <PollCard key={index} poll={poll} onView={handleViewPoll} />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
              <div className="text-center p-6 bg-white rounded-2xl shadow-lg">
                ...
                <p className="text-gray-600">Create polls in minutes with our intuitive interface</p>
              </div>
              <div className="text-center p-6 bg-white rounded-2xl shadow-lg">
                <Clock className="w-12 h-12 text-purple-500 mx-auto mb-4" />
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Time Control</h3>
                <p className="text-gray-600">Set precise start and end times for your polls</p>
              </div>
              <div className="text-center p-6 bg-white rounded-2xl shadow-lg">
                ...
                <p className="text-gray-600">Organize polls by departments or teams</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Design your poll and let people vote!</p>
                </div>
              </div>
              <button
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors ${
                    errors.organization ? 'border-red-500' : 'border-gray-200 focus:border-purple-500'
                  }`}
                />
                    ...
                    <div key={index} className="flex items-center space-x-2">
                      <div className="flex-1">
                        <input
                          ...
                          className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors ${
                            errors[`option${index}`] ? 'border-red-500' : 'border-gray-200 focus:border-purple-500'
                          }`}
                        />
                        {errors[`option${index}`] && (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors ${
                      errors.startTime ? 'border-red-500' : 'border-gray-200 focus:border-purple-500'
                    }`}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-colors ${
                      errors.endTime ? 'border-red-500' : 'border-gray-200 focus:border-purple-500'
                    }`}
                  />
              ...
              <div className="flex space-x-4 pt-4">
                <button
                  onClick={handleCancel}
                  className="flex-1 bg-gray-200 text-gray-700 font-semibold py-3 px-6 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleSubmit}
                  className="flex-1 bg-gradient-to-r from-purple-500 to-blue-500 text-white font-semibold py-3 px-6 rounded-lg hover:from-purple-600 hover:to-blue-600 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-[1.02]"
                >
                  Create Poll
                </button>
              </div>
